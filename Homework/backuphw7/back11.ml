    ...
    let result =
      ...
      | Let (ds, e) -> match ds with 
        ...
        | h::t ->   (match h with 
            ...
            | Val (e1, n) | ByName (e1, n) -> eval (subst (e1, n) e)
                                              
            | Valtuple (e1, l1) -> (match eval e1 with

                | Tuple (l2) -> if List.length l1 <> List.length l2 then stuck 
                        "Wrong tuple length"
                                                                    
                    else 
                    
                      let rec tuplesubstitute l1z l2z ex = match (l1z, l2z) with 
                      
                        | ([], _) -> ex
                        | (y::g, i::z) -> tuplesubstitute g z (subst (i, y) ex)
                        | _ -> ex
                                          
                      in let res = tuplesubstitute l1 l2 e in eval res

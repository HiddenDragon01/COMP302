...
let rec infer (ctx : context) (e : exp) : typ =  match e with
  ...
  | Primop (o, li) -> (match o with 
      ...
      | Negate -> (match li with 
          | [el] -> (match infer ctx el with 
              ...
              | TInt -> TInt
              | _ -> type_fail "Expected an int"
                       
            )
            
          | _ -> type_fail "Not one argument"
  ...
  | Fn (n, _, e1) -> TArrow (ctx_lookup ctx n, infer ctx e1)
                      
  | Rec (_, _, e1) -> infer ctx e1
                       
  | Let (li,e1) ->
      
      let rec fold_ex l1z l2z =
        ...
        | gx::tx -> (match gx with  
            ...
            | Val (ex, na) -> fold_ex tx (union l2z [(na,infer ctx ex)] )
            | Valtuple (ex, na) -> (match infer ctx ex with
        
                | TProduct (l3) -> if List.length li <> List.length l3 then
                      type_fail "Tuple sizes need to be the same"
                
                    else let rec combinehelp l1x l2x l3x = match (l1x, l2x) with
                
                        | ([], _) -> l3x
                        | (g::t), (i::h) -> combinehelp t h (union l3x [(g, i)])
                        | _ -> l3x
                   
                      in fold_ex tx ( union l2z (combinehelp na l3 []) 
            ...
            | ByName (ex, na) -> fold_ex tx (union l2z [(na,infer ctx ex)] )
                
          )
        
      in infer (extend_list ctx (fold_ex li [])) e1
  ...
  | Apply (e1, e2) -> (match infer ctx e1 with 
      ...
      | _ -> type_fail "Need a function" )
               
  | Var (n) -> ctx_lookup ctx n
                 
  | Anno (e1, _) -> infer ctx e1

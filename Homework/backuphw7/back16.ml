...
let rec unify (ty1 : typ) (ty2 : typ) : unit =
  ...
  if typ_eq ty1 ty2 then () else match (ty1, ty2) with
    ...
    | TVar r, TVar r' -> ( match (!r, !r') with
        ...
        | Some (hz), Some (hz') -> if typ_eq hz hz' then () else 
              type_fail "Could not unify"
    ...
    | TVar r, t2' -> if freebies t2' r then r := Some (t2') else  
          type_fail "In free varibles, fail!"
            
    | t2', TVar r-> if freebies t2' r then r := Some (t2') else  
          type_fail "In free varibles, fail!"

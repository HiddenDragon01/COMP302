...
let rec unify (ty1 : typ) (ty2 : typ) : unit =
  ...
  let rec freebies t' a' = match t' with
    ...
    | TVar r -> if r == a' then type_fail "In free varibles, fail!" else true
  ...
  if typ_eq ty1 ty2 then () else match (ty1, ty2) with
    ...
    | TVar r, TVar r' -> ( match (!r, !r') with
        ...
        | Some (gx'), None -> (try if freebies gx' r' then () else r' := !r
                               with TypeError("In free varibles, fail!") -> ()
                              )
        | None, Some (gy')  ->  (try if freebies gy' r then () else r := !r'
                                 with TypeError("In free varibles, fail!") -> ()
                                )
        | None, None -> ()

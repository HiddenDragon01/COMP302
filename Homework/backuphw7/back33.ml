...
let rec unused_vars (e : exp) : name list =  match e with
  ...
  | Fn (x, _, e1) -> let u = (unused_vars e1) and f = (free_vars e1) in
      ...
      if member x f then u
      else x :: u
                       
  | If (e1, e2, e3) -> (unused_vars e1) @ ((unused_vars e2) @
                                           (unused_vars e3))
  | Primop (_, args) -> List.fold_left 
...
let rec subst ((e', x) : exp * name) (e : exp) : exp =
        ...
        (match l1 with 
         ...
         | h::t -> match h with 
           ...
           | Val (y, n) ->  
               ...
               if member n f then let z = fresh_var n in 
                 helper2 
  ...
  | Fn (y, t, e) -> let f = free_vars e' in 
      ...
      if y = x then Fn (y, t, e) else if 
        ...
        member y f then let z = fresh_var y in 
      ...
      | Let (ds, e) -> 
          ...
          let rec evalet l1' l2' = 
            ...
            | h::t ->   (match h with 
                ...
                | Val (e1, n)  -> 
                    evalet t (l2' @ ([eval (replacing l2' e1), n]))
                                    
                | ByName (e1, n) -> 
                    evalet t (l2' @ ([replacing l2' e1, n]))
                                              
                | Valtuple (e1, lz') -> (match eval e1 with

                    | Tuple (l2z') -> if List.length lz' <> List.length l2z'
                        then 
                          stuck 
                            "Wrong tuple length"
...
let rec unify (ty1 : typ) (ty2 : typ) : unit =
  ...
  if typ_eq ty1 ty2 then () else match (ty1, ty2) with
    ...
    | TVar r, TVar r' -> ( match (!r, !r') with
        ...
        | Some (gx'), None -> if freebiesvar gx' r' false then r':= !r 
            else ()
                  
        | None, Some (gy')  ->  if freebiesvar gy' r false
            then  r := !r' else ()
...
let rec infer (ctx : context) (e : exp) : typ =  match e with
  ...
  | If (e1, e2, e3) -> (unify (infer ctx e1) TBool; unify (infer ctx e2) (infer ctx e3);
                        infer ctx e2 
  ...
  | Primop (o, li) -> (match o with 
      ...
      | LessThan | LessEqual | GreaterEqual | GreaterThan | Equals | NotEquals
        ...
        -> if (List.for_all 
                 (fun x -> (unify (infer ctx x) TInt; true)) li ) 
  ...
  | Fn (n, ctx2, e1) -> (match ctx2 with
      ...
      | None -> let a8' = fresh_tvar() in 
          ...
          TArrow (infer (extend ctx (n, a8')) (Var n) , 
                  advanced)
    )
...
| Apply (e1, e2) -> (match infer ctx e1 with 
    ...
    | TArrow (t1, t2) ->(unify (infer ctx e2) t1;
                         t2
                        )

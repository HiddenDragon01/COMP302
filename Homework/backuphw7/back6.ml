  ...
  | Let (list, exp) -> 
      ...
      let rec helper l1 l2 l3 = match l1 with
        ...
        | x::t -> match x with 
          ...
          | Val(e1, n) -> 
              helper t (union l2 [n]) (union l3 (free_vars e1)) 
  ...
  | Let (list, e1) -> 
      ...
      let rec helper1 l1 l2 l3 = match l1 with
        ...
        | x::t -> match x with 
          ...
          | Val(e2, n) -> 
              helper1 t (delete (free_vars e2) (union l2 [n]) )
                (delete [n]
                   (union l3 (unused_vars e2)) )
...
let rec subst ((e', x) : exp * name) (e : exp) : exp =
  ...
  | Let (ds, e2) -> let f = free_vars e' in 
      ...
      let rec replacing list expr = match list with
        ...
        | i::t -> let (b, c) = i in replacing t (subst (b, c) expr) in 
      
      let rec helper2 l1 l2 replace = 
        ...
        (match l1 with 
         ...
         | h::t -> match h with 
           ...
           | Val (y, n) ->  
               ...
               if List.mem n f || n = x then let z = fresh_var n in 
                 helper2 
           ...
           | Valtuple (y,n) -> 
               ...
               let rec find l1x l2x l3x = (match l1x with
                   
                   | [] -> (l2x, l3x)
                         
                   | h::t -> if List.mem h f || h = x then let z = fresh_var h in 
                         
                         find t (l2x @ [z]) (l3x @ [(Var(z), h)])
                           
                       else find t (l2x @ [x]) l3x
                 )
                 
               in let (ax, bx) = find n [] [] in 
               
               
               helper2 t ( l2 @ [Valtuple (  (
